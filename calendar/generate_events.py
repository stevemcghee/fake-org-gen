...
def generate_event_definitions(months=6, num_events_range=(20, 40)):
    """Generates a list of event definitions (time, type, attendees) without creating the final calendar object."""
    shared_event_types = [
    ...
    event_definitions = []
    max_attempts = 1000 # Increased attempts to find slots
    ...
    for _ in range(random.randint(*num_events_range) * months):
        for attempt in range(max_attempts):
            ...
            if is_overlapping(start_time, end_time, event_definitions):
                continue
            ...
            print("Warning: Could not find a free slot after max attempts. Stopping event generation.")
...
def create_calendar_from_definitions(event_definitions, domain):
    ...
    users = {
        "gandalf": f"gandalf the grey <gandalf@{domain}>",
        "frodo": f"frodo baggins <frodo.baggins@{domain}>"
    }
    ...
    for definition in event_definitions:
        ...
        for attendee_key in definition["attendees"]:
            e.add_attendee(users[attendee_key])
...
if __name__ == "__main__":
    domains = ["cpets.joonix.net", "shireholdings.onmicrosoft.com"]
    
    # 1. Generate a single set of event definitions
    ...
    for domain in domains:
        ...
        # Get the directory where the script is located and use it as the output path
